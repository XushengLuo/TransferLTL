...
class problemFormulation(object):
    def __init__(self, case):
        ...
        if case == 1:
            ...
            # self.no = []
            self.formula = '[]<> (e1 && <> e2) && []<> (e3 && <> e4) && <> e5  && <> e6 && !e3 U e1 && !e3 U e1 && !e5 U e1'
            # self.formula = '<> e4 && []<> (e3  && <> e1) && (!e1 U e2)  && []!e5'
            # self.formula = '[]<> e1 && <> e2 && <> e3 && <> e4'
            # self.formula = '[] (<> e1 && <> e2 && <> e3 ) && <> e5 && !e1 U e2 && !e3 U e2 && !e5 U e1'
            self.formula_comp = {1: '(l1_1)',
                                 2: '(l2_1)',
                                 3: '(l3_1)',
                                 4: '(l4_1)',
    ...
    def Formulation(self):
        ...
        return self.workspace, self.regions, self.centers, self.obs, self.init_state, self.uni_cost, self.formula, \
               self.formula_comp, self.exclusion
